...
import com.glowingavenger.plan.problem.Question;
...
public class DialogAgent implements PlanStateChangeListener {
    ...
    public ActionEdge onStateChange(@Nullable Map<String, Boolean> stateBefore, Map<String, Boolean> state,
        ...
        Map<Question, Tuple<ActionEdge, ActionEdge>> askActions = new HashMap<>();
        Map<LogicAction, ActionEdge> realActions = new HashMap<>();

        for (ActionEdge sc : successors) {
            if (sc.action() instanceof Question) {
                if (askActions.containsKey(sc.action())) {
                    askActions.put((Question) sc.action(), new Tuple<>(askActions.get(sc.action()).one, sc));
                } else {
                    askActions.put((Question) sc.action(), new Tuple<ActionEdge, ActionEdge>(sc, null));
                }
        ...
        if (!askActions.isEmpty()) {
            for (Map.Entry<Question, Tuple<ActionEdge, ActionEdge>> entry : askActions.entrySet()) {
