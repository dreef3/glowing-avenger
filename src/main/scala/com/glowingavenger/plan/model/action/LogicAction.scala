...
class LogicAction(val precond: BoolExp, val effect: BoolExp, actionName: String) extends Action {
  ...
  private lazy val precondState = BeliefState(precondModels)
  private lazy val effectModels = Model.retrieveModelsOrThrow(effect)

  override def result(state: BeliefState): BeliefState = if (applicableIn(state)) BeliefState(state.predicates ++ effectModels) else state
  override def applicableIn(state: BeliefState): Boolean = precondState includes state

  override lazy val attrs: List[Symbol] = (effectModels ++ precondModels).keySet.toList
  override val name: String = actionName


  override def toString: String = if (!name.isEmpty) name else s"$precond -> $effect"
